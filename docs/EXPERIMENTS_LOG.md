...
## 🔧 Gold v2 (2026) — sửa 5 span theo `docs/ANNOTATION_GUIDELINE.md`, audit đầy đủ trong §5 file đó

Đối chiếu **toàn bộ 20 span THUỐC** + **8 span TRIỆU_CHỨNG có bổ ngữ** với quy tắc adjacency
(liều/đường dùng gộp vào span chỉ khi nối liền không dấu phẩy/hai chấm/động từ — suy ra trực tiếp
từ 2 ví dụ chính thức của đề). Kết quả: **19/20 THUỐC + 8/8 TRIỆU_CHỨNG-bổ-ngữ đã đúng sẵn** — rút
lại nhận định trước đó rằng gold "lệch hẳn convention đề bài" (số liệu thô 15% có liều gây hiểu
lầm; phần lớn thuốc trong text vốn không có liều nằm sát, không phải annotator bỏ sót).

**5 span thực sự sửa** (file `scripts/gold_annotations.py`, rebuild bằng `python -m scripts.build_gold`):

| file | trước | sau | lý do |
|---|---|---|---|
| 91 | `"heparin"` | `"heparin truyền tĩnh mạch liên tục"` | đường dùng+tần suất nối liền, không dấu phẩy/động từ |
| 51 | `"cảm thấy khó chịu chung"` | `"khó chịu chung"` | loại động từ dẫn `"cảm thấy"` khỏi span |
| 91 | `"Không chảy máu mũi"` | `"chảy máu mũi"` (assertions giữ `isNegated`) | cue phủ định KHÔNG nằm trong text (đối chiếu ví dụ đề: `assertion:"isHistorical"` không chép chữ "tiền sử" vào text) |
| 3 | `"Không đánh trống ngực"` | `"đánh trống ngực"` | như trên |
| 3 | `"Không chóng mặt"`, `"Không buồn nôn"` | `"chóng mặt"`, `"buồn nôn"` | như trên |

**Hiệu ứng đo bằng `scripts/diagnose_boundary.py --all-types`** (so predictions exp_0022 với gold,
không phải chấm điểm final — đây là công cụ chẩn đoán riêng, không dùng `src/eval/metric.py`):

| | trước | sau |
|---|---|---|
| exact (đúng type + đúng 2 biên) | 95 (58.6%) | **100 (61.7%)** |
| boundary (đúng type, sai biên) | 31 (19.1%) | 26 (16.0%) |
| lệch biên TRÁI | 10 | **5** |

Cải thiện sạch, đúng bằng 5 span sửa, không tác dụng phụ ngoài ý muốn (lệch phải/cả-hai không đổi).

⚠️ **Chưa xử lý** (ngoài phạm vi đợt sửa boundary này, thuộc vấn đề *completeness*): file 50 có
`"prednisone 40 mg"` xuất hiện lần 2 trong text nhưng không được gán nhãn — occurrence bị bỏ sót,
xem [IDEAS.md](IDEAS.md) "Hoàn thiện dev gold".

**Quyết định lưu trữ**: sửa tại chỗ (`data/labeled/ground_truth/`), không tạo `_v2/` song song —
git giữ lịch sử để đối chiếu khi cần. Mọi điểm dev ghi TRƯỚC dòng này trong bảng bên dưới dùng
gold v1 (đã có 5 sai lệch nhỏ trên); không cần chấm lại các experiment cũ vì các sai lệch quá nhỏ
để đổi kết luận xếp hạng đã rút ra, nhưng nếu re-run `scripts/rescore_all.py` cho experiment mới,
kết quả sẽ dùng gold v2 này.

## 🔧 exp_0024 — sửa 2 bug hậu xử lý biên NER (A2/A3, `docs/TRAINING_WORKFLOW_PLAN.md`)

Hai bug đo được qua `diagnose_boundary.py` (đối chiếu exp_0022 predictions vs gold v2), sửa trong
`src/extraction/ner_extractor.py`:

1. **`_snap_word` nuốt mất chữ số trước dấu thập phân**: model trúng `'2'` trong `'5.2'` →
   `_snap_word` cũ dừng ngay tại `.` (không có trong `_WORDCH`), trả về `'2'` thay vì `'5.2'`.
   Sửa bằng vòng lặp mở rộng riêng cho pattern `digit [.,] digit`, KHÔNG sửa `_WORDCH` chung (sẽ
   phá ranh giới câu ở chỗ khác dùng nó).
2. **Vươn qua ngoặc chứa nội dung tường thuật**: `'prograf (dose decreased from 5mg bid to 1mg
   bid)'` thay vì `'prograf'`. Hàm mới `_trim_narrative_paren()`: cắt khi ngoặc CHƯA đóng trong
   span (luôn là artifact cắt ngang) hoặc ngoặc đã đóng nhưng nội dung có cue tường thuật
   (`decreased`, `giảm liều`, `tiền sử`...). **Giữ nguyên** ngoặc là 1 phần tên (`"Rosuvastatin
   (Crestor)"`) — không có cue nào khớp. Áp cho `CHẨN_ĐOÁN`/`THUỐC` (nơi đo được lỗi).

| `diagnose_boundary --all-types` | exp_0022 (gold v2) | exp_0024 (sau fix) |
|---|---|---|
| exact | 100 (61.7%) | **110 (67.9%)** |
| boundary | 26 (16.0%) | **22 (13.6%)** |
| lệch trái | 5 | 3 |
| `KẾT_QUẢ_XÉT_NGHIỆM` exact | 5/22 | **13/22** — đúng chỗ bug decimal tác động mạnh nhất |

| dev score | text | assert | cand | final |
|---|---|---|---|---|
| exp_0022 (gold v2) | 0.2463 | 0.6365 | 0.3333 | 0.3982 |
| **exp_0024** | **0.2577** | 0.6328 | 0.3333 | **0.4005** |

`cand` đứng yên — hai bug chỉ đụng `text`/boundary, chưa đổi đủ nhiều case để ảnh hưởng mã tra
được qua reranker. `spurious` tăng nhẹ 311→328 (fix decimal đôi khi tách 1 span dài thành nhiều
span ngắn khớp đúng số, sinh thêm vài match lệch chỗ khác) — chấp nhận vì net effect dương rõ.

⏳ Chưa nộp BTC — đợi gộp thêm kết quả synthetic v5 (Phase 2) để dùng 1 lượt nộp đo tổng hợp.

## 🧪 exp_0025 — NER trên synthetic v5 (SYNTHETIC_V5_PLAN.md), 2 biến thể so trên dev

Cùng reranker (`BAAI/bge-reranker-v2-m3`) + assertion classifier với exp_0024, chỉ đổi NER model,
để cô lập ảnh hưởng của synthetic v5 (quality-first, transductive frame mining) so với
`ner_xlmr_v4_mix` (8830 doc cũ).

| exp | NER train trên | #doc | dev text | dev assert | dev cand | dev final |
|---|---|---|---|---|---|---|
| exp_0024 (nền so sánh) | `train_v4_mix` | 8830 | 0.2577 | 0.6328 | 0.3333 | 0.4005 |
| exp_0025_v5a | `train_v5a` (frame~85%, prose~15% entity) | 572 | 0.2589 | 0.5964 | 0.3061 | 0.3790 |
| exp_0025_v5c | `train_v5c` (full pool: frame 1000 + prose 269) | 1269 | 0.2504 | 0.6612 | 0.3014 | 0.3941 |

**`diagnose_boundary.py --all-types`** (đối chiếu span thật, không qua metric có cơ chế abstain):

| | v4_mix (8830) | v5a (572) | v5c (1269) |
|---|---|---|---|
| exact | 110 (67.9%) | 90 (55.6%) | 97 (59.9%) |
| missed | 23 (14.2%) | 38 (23.5%) | 35 (21.6%) |
| wrong_type | 7 (4.3%) | 11 (6.8%) | 10 (6.2%) |
| spurious | 328 | 263 | 321 |

**Kết luận**: dùng full pool (1269) thay vì subset theo tỉ lệ entity (572) cải thiện thật —
`exact` 55.6%→59.9%, `missed` 23.5%→21.6% — xác nhận giả thuyết "thêm doc cùng cách sinh giúp
recall". Nhưng **v5c vẫn chưa vượt v4_mix** (8830 doc): kém 8 điểm % ở `exact`, và `spurious`
gần quay lại mức v4_mix (321 vs 328) — dùng hết 1000 frame-doc kéo theo lại phần mật độ entity
cao hơn tối ưu. → **hướng quality-first (v5) đang đi đúng chiều nhưng cần thêm quy mô** (kế
hoạch dự phòng: sinh tới ~2000 doc) mới kết luận được có vượt quantity-baseline hay không.

⏳ Cả 2 chưa nộp BTC ở tập test cũ — BTC cấp đề mới (turn 2) trước khi kịp nộp, xem mục dưới.

## 📦 BTC cấp lại đề — TURN 2 (2026-07-23) — LẦN NÀY LÀ THẬT, khác hẳn "nâng cấp giả" 2026-07-16

⚠️ Phân biệt với mục "nâng cấp đề" ngay dưới đây (2026-07-16): lần đó input **giống hệt byte-for-byte**.
Lần này (`data/raw_turn_2/input_turn2_vong1.zip`, giải nén → `data/raw_turn_2/input/`) input
**THẬT SỰ khác**: đã verify **0/100 file trùng byte** với `data/raw_new/input/` cũ, và n-gram-8
so khớp file[i] mới với CHÍNH file[i] cũ chỉ **median 0.0%, max 1.1%** — case bệnh nhân hoàn
toàn mới (không phải case cũ chèn thêm nhiễu). Chi tiết đầy đủ + số liệu: `docs/EDA_FINDINGS.md`
§0 (tổng ký tự +54%, xuất hiện redaction thuốc bằng `***` ở 30/100 file, nội dung lạc đề chèn
giữa case ở 37/100 file — cả hai hiện tượng **hoàn toàn không có** ở input cũ).

**Quyết định**: `data/raw_turn_2/input/` là input để sinh submission thật từ nay.
`data/raw_new/`/`data/raw/` giữ nguyên để đối chiếu lịch sử.

### exp_0026 / exp_0027 — chạy lại 2 model tốt nhất trên test turn 2

Cùng cấu hình hệt exp_0024/exp_0025_v5c (không train lại gì), chỉ đổi `--test-input
data/raw_turn_2/input`. Dev score (đo trên `data/labeled/` cũ, không đổi) do đó **giống hệt**
exp_0024/exp_0025_v5c — giá trị của 2 experiment này nằm ở `predictions_test`/`output.zip` mới,
không phải ở dev score.

| exp | = | NER | test-input | output.zip |
|---|---|---|---|---|
| `exp_0026_v4mix_turn2` | exp_0024 | `ner_xlmr_v4_mix` | `data/raw_turn_2/input` | 100 file, 1858 span (18.6/file) |
| `exp_0027_v5c_turn2` | exp_0025_v5c | `ner_xlmr_v5c` | `data/raw_turn_2/input` | 100 file, 2042 span (20.4/file) |

Phân bố type dự đoán trên test turn 2:

| | CHẨN_ĐOÁN | TRIỆU_CHỨNG | TÊN_XÉT_NGHIỆM | KẾT_QUẢ_XÉT_NGHIỆM | THUỐC |
|---|---|---|---|---|---|
| exp_0026 (v4_mix) | 425 | 790 | 358 | 127 | 158 |
| exp_0027 (v5c) | **677** | 707 | 319 | 195 | 144 |

v5c đoán `CHẨN_ĐOÁN` nhiều hơn hẳn (677 vs 425) — có thể phản ánh việc train nhiều trên khung
`- {CHẨN_ĐOÁN}` bare-bullet (tần suất cao nhất trong 49 khung mined) khớp tốt với input turn 2
(nhiều file dạng bài giáo dục liệt kê bệnh). Chưa biết đúng/sai — chỉ BTC mới trả lời được.

**Verify hành vi trên vùng redaction `***`**: cả 2 model **không tag được entity nào** gần vị
trí `***` trong file `1.txt` — không có tín hiệu lexical để nhận diện, không phải bug.

### 🏆 ĐIỂM THẬT BTC trên test turn 2 (nộp 2026-07-23)

| exp | WER | J_assertion | J_candidates | final |
|---|---|---|---|---|
| **exp_0026 (v4_mix)** | 72.1682 | 30.33 | 13.2999 | **22.7685** |
| exp_0027 (v5c) | 73.5271 | 27.3107 | 9.7464 | 20.0336 |

Công thức khớp cả 2 (22.76850≈22.7685, 20.03364≈20.0336) — xác nhận lần thứ N liên tiếp.
Top 1 leaderboard hiện tại: **40.5**.

**🔑 Phát hiện 1 — test turn 2 khó hơn hẳn cho MỌI model, không phải lỗi cục bộ.** So
`exp_0026` (v4_mix + boundary fix) với `exp_0022` (v4_mix gốc, cùng NER, **test cũ**):

| | exp_0022 (test cũ) | exp_0026 (test turn 2) | chênh |
|---|---|---|---|
| WER | 61.3095 | 72.1682 | +10.86 (tệ hơn) |
| J_assertion | 39.3464 | 30.33 | −9.02 |
| J_candidates | 23.4669 | 13.2999 | **−10.17 (−43% tương đối)** |
| final | 32.798 | 22.769 | **−10.03** |

Cả 3 thành phần đều tụt đồng loạt — khớp đúng dự báo từ `EDA_FINDINGS.md` §0 (văn phong bài
giáo dục sức khỏe, redaction thuốc, nội dung lạc đề, tài liệu dài hơn 54%). `J_candidates` bị
đánh nặng nhất theo tỉ lệ tương đối — hợp lý vì candidate mapping dựa vào khớp text với KB, mà
văn phong bách khoa diễn đạt tên bệnh/thuốc rất khác ghi chú lâm sàng mà SapBERT/reranker được
tối ưu cho.

**🔑 Phát hiện 2 — xếp hạng v4_mix > v5c GIỮ NGUYÊN qua dịch chuyển phân phối.** Trên CÙNG test
turn 2: v4_mix thắng v5c ở cả 3 thành phần (`J_candidates` 13.30 vs 9.75, −27% tương đối cho
v5c). Đây đúng thứ tự `diagnose_boundary` đã dự đoán trên test CŨ (`exact` 67.9% > 59.9%) —
**bằng chứng độc lập thứ 2 rằng phương pháp xếp hạng qua `diagnose_boundary` đáng tin cậy, kể cả
khi dữ liệu dịch chuyển phân phối mạnh.** Kết luận "v5 cần thêm quy mô, chưa vượt quantity-baseline"
được xác nhận thêm lần nữa, độc lập với BTC thật.

## 🔧 exp_0028/exp_0029 — KB-description reranker (C1/C2 TRAINING_WORKFLOW_PLAN.md) — ❌ loại bỏ

Giả thuyết: cross-encoder rerank bằng **mô tả KB** (phân cấp ICD + đồng nghĩa RxNorm, xây từ
`icd10_vn.csv`/`rxnorm_terms.csv`, 100% rule-based, không LLM) đáng lẽ giúp disambiguate tốt hơn
tên trần — đúng hướng OpenBioNER (description-conditioning giúp candidate/linking, không giúp NER).
Cùng NER (`ner_xlmr_v4_mix`+boundary-fix, **giống span exp_0026** → dev đáng tin cậy trực tiếp,
không cần `diagnose_boundary`).

| exp | mô tả | max_length | dev candidates_score |
|---|---|---|---|
| exp_0026 (baseline) | tên trần | 64 | **0.3333** |
| exp_0028 | + mô tả KB | 64 (bug: cắt cụt mô tả dài) | 0.3011 (**−9.7%**) |
| exp_0029 | + mô tả KB | 160 (fix truncation) | 0.2903 (**−12.9%, còn TỆ hơn**) |

Giả thuyết "truncation là nguyên nhân" **sai** — tăng `max_length` không cứu được mà còn tệ hơn.
Kết luận: mô tả KB càng dài (thêm chương/khối/đồng nghĩa) càng làm loãng tín hiệu relevance của
cross-encoder **off-the-shelf** (chưa fine-tune) — nó không biết cách cân trọng phần mô tả phụ
(hierarchy) so với tên chính. Có thể sẽ có tác dụng NẾU fine-tune cross-encoder trên cặp KB
(Module C3/C5, 211,890 cặp) nhưng việc đó không khả thi trong ngân sách thời gian hiện tại.
→ **`RerankMatcher.use_description` default đã revert về `False`** (`src/normalization/reranker.py`),
bảo toàn cấu hình đã biết tốt (exp_0026). exp_0029 không chạy hết `predictions_test`/không đóng
gói `output.zip` — không dùng để nộp, chỉ dùng dev score sẵn có từ `predictions/` (đã đủ kết luận,
không tốn thêm compute cho 100 file test).

## 🔧 exp_0030 — Entity masking A/B (A2 TRAINING_WORKFLOW_PLAN.md) — ❌ không cải thiện rõ

Ý tưởng OpenBioNER: che TOÀN BỘ token của 1 entity bằng `[MASK]` với xác suất `p` lúc train
(`scripts/train_ner.py --mask-prob`, áp theo TỪNG entity — nhóm B-X liền I-X cùng type — không
phải từng token rời), ép model dựa ngữ cảnh thay vì học thuộc mặt chữ. Train trên `train_v5c`/
`val_v5c` (1269/189 doc, val KHÔNG mask). `ner_xlmr_v5c` (p=0, đã có sẵn) làm control.

Val F1 (đo trên val SẠCH, cùng phân phối train) giảm dần theo `p` — đúng dự đoán (nhiệm vụ train khó
hơn): p=0 ≈0.9998, **mask01 (p=0.1) 0.9818**, **mask03 (p=0.3) 0.9313**.

**`diagnose_boundary.py --all-types` (span khác NER → không tin trực tiếp dev, phải so theo cách này):**

| | v5c (p=0) | mask01 (p=0.1) | mask03 (p=0.3) |
|---|---|---|---|
| exact | 59.9% | 58.6% | 55.6% |
| boundary | 12.3% | 14.2% | 12.3% |
| wrong_type | 6.2% | 8.6% | 8.0% |
| missed | 21.6% | **18.5% (tốt hơn)** | 24.1% |
| spurious | 321 | 302 | 291 |

**Dev pipeline đầy đủ (NER+reranker+assertion_clf, test-input=dev để so nhanh, KHÔNG dùng nộp):**

| | v5c (p=0) | mask01 | mask03 |
|---|---|---|---|
| text_score | 0.2504 | 0.2474 | 0.2302 |
| assertions_score | 0.6612 | **0.5514 (tệ hơn hẳn)** | 0.5941 |
| candidates_score | 0.3014 | **0.3302 (tốt hơn)** | 0.2814 |
| final_score | 0.3941 | 0.3717 | 0.3598 |

**Kết luận**: `mask01` cho tín hiệu **trái chiều** — `missed`/`spurious`/`candidates_score` tốt
hơn (đúng giả thuyết: bớt học thuộc mặt chữ → tổng quát hoá tốt hơn cho linking) nhưng
`wrong_type`/`assertions_score` tệ hơn rõ rệt, kéo `final_score` xuống. `mask03` tệ ở hầu hết mọi
mặt (masking quá nhiều làm mất tín hiệu cần thiết để phân loại type). Với n=162 concept trên 15
dev file, biên độ nhiễu không nhỏ — nhưng **không có bằng chứng masking cải thiện tổng thể**, và
dù `mask01` có thắng ở candidates, `v5c` gốc (nền để test masking) đã **thua `v4_mix` trên BTC
thật** (20.03 vs 22.77) — nên masking-trên-v5c không đổi kết luận cuối: **`v4_mix` (exp_0026)
vẫn là cấu hình tốt nhất đã xác nhận**. Không train lại masking trên `v4_mix` (8830 doc, chi phí
train cao hơn nhiều so với v5c 1269 doc, không đủ thời gian) — để ngỏ cho lần sau.

## 🔬 exp_0031 — Position ablation: field `position` quan trọng đến đâu?

Lấy y nguyên `exp_0026` (best confirmed, BTC 22.7685), ghi đè toàn bộ `position` → `[0, 0]`,
giữ nguyên `text`/`type`/`candidates`/`assertions`. Không train lại gì — chỉ hậu xử lý output JSON
đã có. Chi tiết đầy đủ + diễn giải cơ chế: `experiments/exp_0031_position_ablation/notes.md`.

Đo trên dev (gold thật, đáng tin — `_match()` chỉ ghép fallback qua IoU vị trí khi text KHÔNG khớp
tuyệt đối, nên `position=[0,0]` triệt tiêu mọi fallback):

| | gốc | position=[0,0] | delta |
|---|---|---|---|
| text_score | 0.2577 | 0.2340 | −0.0237 |
| assertions_score | 0.6328 | 0.6318 | −0.0010 |
| candidates_score | 0.3333 | 0.3191 | −0.0142 |
| final_score | 0.4005 | 0.3874 | **−0.0131 (−3.3%)** |

**Kết luận (trên proxy nội bộ, dev)**: position đóng vai trò PHỤ — phần lớn credit đến từ text
chuẩn hoá khớp tuyệt đối, chỉ thiểu số cặp cần fallback IoU vị trí để ghép được.

### 🚨 ĐÃ NỘP BTC — kết quả THẬT khác HẲN dự đoán dev (2026-07-24)

| | exp_0026 (position thật) | exp_0031 (position=[0,0]) | delta |
|---|---|---|---|
| WER | 72.1682 | **100** | +27.83 (tệ hơn) |
| J_assertion | 30.33 | **0** | −30.33 |
| J_candidates | 13.2999 | 19.33 | +6.03 (tăng, xem giải thích) |
| final | **22.7685** | **~7.73** | **−15.04 (−66% tương đối)** |

**Dev proxy dự đoán −3.3%, BTC thật −66%.** Sai lệch độ lớn RẤT LỚN — bằng chứng rõ ràng scorer
BTC thật dựa vào `position` để ghép cặp predict↔gold **chặt hơn nhiều** so với `_match()` nội bộ
(text-match OR position-IoU>0.5, coi position chỉ là fallback nhẹ). Với `position=[0,0]` toàn bộ,
gần như KHÔNG concept nào ghép được → `WER=100` (mismatch toàn bộ) + `J_assertion=0` (mọi gold
coi như missed). `J_candidates` lại TĂNG (13.30→19.33) — giả thuyết: cơ chế "abstain ăn `J(∅,∅)=1`"
(đã kiểm chứng ở exp_0012) vẫn áp dụng cho các pred `candidates=[]` dù đã mất credit vị trí, nên
không phải bằng chứng position "không quan trọng" cho candidates.

**Hệ quả quan trọng cho hướng đi tiếp theo**: độ chính xác BOUNDARY của NER (position chính xác)
có giá trị THẬT cao hơn nhiều so với những gì dev proxy từng thể hiện — nên **tiếp tục ưu tiên các
cải tiến boundary-precision** (`_snap_word`/`_trim_narrative_paren`, `diagnose_boundary.py`) thay vì
coi đó là cải tiến nhỏ. Đã thêm cảnh báo vào `src/eval/metric.py` docstring — **dev proxy hiện tại
ĐÁNG TIN cho XẾP HẠNG tương đối giữa các model bình thường (đã kiểm chứng nhiều lần), nhưng KHÔNG
mô phỏng đúng mức độ ảnh hưởng của lỗi position nặng/cực đoan.**

Chi tiết đầy đủ + giả thuyết: `experiments/exp_0031_position_ablation/notes.md`.
